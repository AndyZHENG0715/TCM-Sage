...
- 若检索到同一方剂或概念的多个版本，应以最经典、最早出处为主，其余版本可作补充说明，而非简单罗列所有版本。
...
def build_prompt_template(system_prompt: str) -> ChatPromptTemplate:
    ...
    return ChatPromptTemplate.from_messages([
        ...
        ("user", (
            ...
            "---\n\n"
            "用户提问：{question}\n\n"
            "请基于以上资料和自身知识，给出详尽、全面的回答。\n"
            "要求：\n"
            "- 对每个要点展开论述，不要简单罗列或压缩为一句话\n"
            "- 每个功效、概念或证型用独立段落详细解释，包括临床应用举例\n"
            "- 善用 ## 标题、**加粗**、- 列表、> 引用等 Markdown 格式组织内容\n"
