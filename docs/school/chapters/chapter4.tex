...
The system is built on a modern full-stack architecture designed for high-performance natural language processing and scalable web delivery.
...
\begin{itemize}
    ...
    \item \textbf{Embedding Model:} \textbf{DashScope text-embedding-v4} is employed for vectorization, producing 1024-dimensional dense embeddings optimized for Chinese semantic retrieval.
    \item \textbf{Reranker:} Post-retrieval ranking is performed using \textbf{qwen3-rerank}, which applies a cross-encoder architecture to refine the initial retrieval results based on query-document relevance.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Alias Mapping:} A crosswalk bridge that translates colloquial terms (e.g., "stomach ache") to formal TCM syndromes (e.g., \textit{Wei Wan Tong}).
...
The generation layer is optimized to ensure the LLM adheres to professional TCM methodologies while maintaining strict formatting.

\subsection{System Prompt Architecture}
We use \texttt{ChatPromptTemplate.from_messages} to maintain a clean separation between the \texttt{SystemMessage} and the \texttt{UserMessage}. Empirical testing showed that merging these into a single string caused the model to ignore complex formatting constraints.
...
The "Plain LLM" baseline uses a generic "helpful assistant" prompt and has access to \textbf{DuckDuckGo Search} (\texttt{ddgs} package) to simulate a modern AI's ability to retrieve information from the general web. This represents the current state-of-the-art for non-specialized models.
...
\begin{itemize}
    ...
    \item \textbf{Citation Panel:} Clicking a citation number opens a side panel displaying the full source text and a \textbf{Cytoscape.js} mini-view of the relevant knowledge graph nodes.
    \item \textbf{KG Explorer:} A dedicated \texttt{/kg/[entityId]} route allows users to browse the 20,000-node graph, expand nodes on click, and search for specific entities.
\end{itemize}
