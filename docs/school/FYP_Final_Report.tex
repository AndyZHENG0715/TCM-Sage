...
\author{Andy Zheng (Student ID: [REDACTED]) \\ 
...
\begin{abstract}
Traditional Chinese Medicine (TCM) practitioners face significant challenges in retrieving and synthesizing evidence from a vast corpus of classical literature. Existing Large Language Models (LLMs) often function as "black boxes," providing unverifiable claims that lack the precision required for clinical or academic reference. This project introduces TCM-Sage, an evidence-synthesis tool designed to provide transparent, citation-backed insights from 17 foundational TCM texts. By implementing a Retrieval-Augmented Generation (RAG) pipeline with hybrid retrieval—combining vector search with the SymMap v2.0 knowledge graph—TCM-Sage ensures every claim is traceable to its original source. Evaluation via a blind A/B Arena system demonstrates statistically significant improvements in evidence quality and reliability compared to general-purpose LLMs.
\end{abstract}
...
The motivation for this project stems from both personal and professional observations. My father first suggested exploring the intersection of AI and medicine, noting the potential for technology to assist in specialized fields. This direction was further reinforced through discussions with Kenny (Hu Shinan), a Year 5 student at the HKBU School of Chinese Medicine with a 3.97 cGPA, currently interning at the Guangdong Provincial Hospital of TCM. Kenny highlighted the critical need for a tool that could provide reliable citations from classical texts, as current digital resources are often fragmented or lack the depth required for serious study.
...
Herbology \& Formulary & Shennong Bencao Jing, Tangye Bencao, Bencao Gangmu (Selected) \\
Jin-Yuan Four Masters & Suwen Xuanji Yuanbing Shi, Yinshan Zhengyao, Piwei Lun, Gezhi Yulun \\
General Clinical & Qianjin Fang, Jingyue Quanshu (Selected) \\
\bottomrule
\end{tabularx}
\label{tab:texts}
\end{table}
...
There is a clear gap in the market and in research for a tool that prioritizes evidence synthesis over direct diagnosis. Practitioners need a "Glass Box" system where every claim is backed by a traceable citation, allowing the human expert to remain the final decision-maker while the AI handles the heavy lifting of literature retrieval.
...
\begin{enumerate}
    ...
    \item \textbf{Local Deployment:} Ensure the system supports full local deployment to address data privacy concerns. Both the embedding models and the LLM (e.g., Qwen-7B or Llama-3) must be capable of running on consumer-grade hardware.
